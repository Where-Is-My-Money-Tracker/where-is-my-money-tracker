// change name to add purchase for component and file
...
class AddPurchaseItem extends Component {
    ...
    componentDidMount = async () => {
        ...
        const childCategories = categories.filter(
            (item) => item.parent_id === this.state.parentCategory
        );
    ...
    handleCategoryChange = async (e) => {
        ...
        const categories = await getCategories();
        const childCategories = categories.filter(
            (item) => item.parent_id === Number(e.target.value)
        );
        this.setState({ childCategories });
    };
    ...
    render() {
        let labelMessage; //what is label message? maybe just category?

        if (Number(this.state.parentCategory) === 0) {
            labelMessage = "Main";
        } else {
            labelMessage = findById(
                this.state.allCategories,
                Number(this.state.parentCategory).description
            );
        }
        ...
        return (
            <div className="addPurchaseForm">
                ...
                <form
                    onSubmit={(e) => this.handleSubmitPurchase(e)}
                ...
                >
                    ...
                    <input
                        ...
                        required
                    ></input>
                    {/* are the two line breaks below necessary? */}
                    <br></br>
                    <label id="dropDownLabel">
                        ...
                        <p>
                            <i>
                                Use the selector below to choose a subcategory
                                of, or add a new subcategory to, {labelMessage}
                            </i>
                        </p>
                    </label>
                    <select
                    ...
                    >
                        ...
                        <option key="new" value="SetNewCategory">
                            Add New Category
                        </option>
                    </select>
                    {this.state.optionSelector === "SetNewCategory" ||
