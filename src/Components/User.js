...
class User extends Component {
    ...
    handleGoBack = async () => {
        const parentId = findById(
            this.state.allCategories,
            Number(this.state.parentCategory)
        ).parent_id;
    ...
    renderChart = async (parentId, timeWindow) => {
        ...
        const childCategories = findByParentId(
            this.state.allCategories,
            parentId
        );
        ...
        // see if this still works after we address recursion bug
        const filteredPurchases = this.state.allPurchases.filter((item) => {
            return item.timestamp
                ? item.timestamp > Date.now() - timeWindow
                : item.stop_timestamp > Date.now() - timeWindow ||
                      item.stop_timestamp === null;
        });
        ...
        const chartData = mungeChartData(
            childCategories,
            this.state.allCategories,
            filteredPurchases
        );
                ...
                <div className="chartDiv">
                    ...
                    {this.state.childCategories.length === 0 ? (
                        ...
                        <article className="chart">
                            <PieChart
                                data={insertChartData(this.state.chartData)}
                            />
                        </article>
                    )}
                    <label id="timeWindow" htmlFor="time window">
                ...
                <div className="dropdown">
                    <details>
                        ...
                        <div className="purchaseLinks">
                            <NavLink to="/addpurchaseitem">
                                Add New Purchase
                            </NavLink>
                            <NavLink to="/addrecurringpurchaseitem">
                                Add Recurring Purchase
                            </NavLink>
                            <NavLink to="/modifyrecurringpurchaseitem">
                                Modify Recurring Purchase
                            </NavLink>
                            <NavLink to="/deletepurchases">
                                Delete a Purchase
                            </NavLink>
                        </div>
                    </details>
